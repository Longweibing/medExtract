...
#	
#	Changelog:
#	10/16/2009 -- Added Pr, Re output option (-p flag)
...
#			print "#WARNING: record %s not present in system file."%g
		...
		n_sh += f_n
	if show_pr:
		hsys = [str(x) for x in [is_exact,"horizontal","system-level",'X',fmeasure(d_sh,s_sh,n_sh),prmeasure(d_sh,s_sh,n_sh)[0],prmeasure(d_sh,s_sh,n_sh)[1]]]
		hpat = [str(x) for x in [is_exact,"horizontal","patient-level",'X',average(file_fms),average([y[0] for y in file_prms]),average([y[1] for y in file_prms])]]		
	else:
		hsys = [str(x) for x in [is_exact,"horizontal","system-level",'X',fmeasure(d_sh,s_sh,n_sh)]]
		hpat = [str(x) for x in [is_exact,"horizontal","patient-level",'X',average(file_fms)]]
	print '\t'.join(hsys)
	print '\t'.join(hpat)
	for mtag in vert_rms:
		if show_pr:
			vsys = [str(x) for x in [is_exact,"vertical","system-level",mtag,fmeasure(vert_sms[mtag]['D'],vert_sms[mtag]['S'],vert_sms[mtag]['N']),prmeasure(vert_sms[mtag]['D'],vert_sms[mtag]['S'],vert_sms[mtag]['N'])[0],prmeasure(vert_sms[mtag]['D'],vert_sms[mtag]['S'],vert_sms[mtag]['N'])[1]]]
			vpat = [str(x) for x in [is_exact,"vertical","patient-level",mtag,average(vert_rms[mtag]),average([y[0] for y in vert_pr_rms[mtag]]),average([y[1] for y in vert_pr_rms[mtag]])]]
		else:
			vsys = [str(x) for x in [is_exact,"vertical","system-level",mtag,fmeasure(vert_sms[mtag]['D'],vert_sms[mtag]['S'],vert_sms[mtag]['N'])]]
			vpat = [str(x) for x in [is_exact,"vertical","patient-level",mtag,average(vert_rms[mtag])]]
		print '\t'.join(vsys)
		print '\t'.join(vpat)
		

def build_batch_files(record_dir,entry_dir,outpath,entry_format="%s.i2b2.entries"):
	...
	for r in records:
		...
		if entry_file in entries:
			fh = open(entry_dir + entry_file,'r')
...
if __name__ == "__main__":
	...
	parser = OptionParser()
	parser.add_option("-x","--xmlmake",action="store_true",dest="do_xmlgen",default=False,help="Generates XML file from specified record and entry directories.")
	parser.add_option("-p","--pandr",action="store_true",dest="show_pr",default=False,help="Prints entry-level precision and recall values")	
	parser.add_option("-r","--recordpath",dest="recordpath",help="Path to the directory containing i2b2 records (only applies if -x is specified)")
	parser.add_option("-z","--entrypath",dest="entrypath",help="Path to the directory containing i2b2 entries (only applies if -x is specified) -- format of entry files expected is: <record_name>.i2b2.entries")	
	parser.add_option("-o","--xmlout",dest="xmlout",help="File from which to create an XML entry file (only applies if -x is specified).")
	parser.add_option("-g","--goldxml",dest="goldxmlp",help="Path to the ground truth XML i2b2 entry file (only applies if -x is omitted).")
	parser.add_option("-s","--sysxml",dest="sysxmlp",help="Path to the system XML i2b2 entry file (only applies if -x is omitted).")
